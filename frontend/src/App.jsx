...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    const err =
      pred && true_ ? Math.abs(pred.value - true_.value).toFixed(3) : null;
    return (
      <div className="bg-[#0d1526] border border-amber-400/30 rounded-xl p-3 md:p-4 font-['Space_Mono'] text-[11px] shadow-2xl">
        ...
        {pred && (
          <p className="text-amber-400 my-1">
            RLS Pred SoC : {pred.value?.toFixed(2)}%
          </p>
        )}
        {true_ && (
          <p className="text-cyan-400 my-1">
            True SoC : {true_.value?.toFixed(2)}%
          </p>
        )}
        {err && (
          <p className="text-[#6b7fa8] mt-2 border-t border-white/10 pt-2">
            Δ Error : {err}%
          </p>
        )}
      </div>
...
function App() {
  ...
  const absError =
    predSoc != null && trueSoc != null ? Math.abs(predSoc - trueSoc) : null;
  const convergence =
    absError != null ? Math.max(0, Math.min(100, 100 - absError * 30)) : 0;
  ...
  const errorTag =
    absError == null
      ? null
      : absError < 0.5
        ? { label: "CONV", text: "text-green-400", bg: "bg-green-400/10" }
        : absError < 1.5
          ? { label: "DVRG", text: "text-amber-400", bg: "bg-amber-400/10" }
    ...
    <>
      ...
      <div className="min-h-screen bg-[#080e1a] text-[#f0f4ff] font-['DM_Sans'] p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            <div className="flex items-center gap-2 px-3 md:px-4 py-2 rounded-full bg-green-400/10 border border-green-400/20 font-['Space_Mono'] text-[10px] md:text-[11px] text-green-400 tracking-wider">
              <span
                className={`w-1.5 h-1.5 rounded-full bg-green-400 ${isSimulating ? "animate-pulse" : ""}`}
              ></span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            {
              label: "RLS Predicted SoC",
              value: predSoc?.toFixed(2) ?? "--",
              unit: "%",
              sub: "recursive estimate",
              tag: {
                label: "+RLS",
                text: "text-green-400",
                bg: "bg-green-400/10",
              },
              accent: "border-t-amber-400/40",
            },
            {
              label: "True SoC (Ground Truth)",
              value: trueSoc?.toFixed(2) ?? "--",
              unit: "%",
              sub: "physics model",
              tag: {
                label: "BMS",
              ...
              accent: "border-t-cyan-400/40",
            },
            {
              label: "Absolute Error",
              value: absError?.toFixed(3) ?? "--",
              unit: "%",
              sub: "live delta",
              tag: errorTag,
              accent: "border-t-green-400/40",
            },
            {
              label: "Internal Resistance Rᵢ",
              value:
                latest?.r_internal != null
                  ? latest.r_internal.toFixed(4)
                  : "--",
              unit: "Ω",
              tag: {
                label: "Rᵢ",
              ...
              sub: "live ΔV/ΔI estimate",
              accent: "border-t-[#6b7fa8]/40",
            },
          ].map((m, i) => (
            <div
              key={i}
              className={`bg-[#111d30] border border-white/5 rounded-xl p-4 md:p-5 border-t-2 ${m.accent} shadow-lg`}
            >
              ...
              <div className="font-['Space_Mono'] text-2xl md:text-3xl font-bold text-[#f0f4ff] leading-none mb-2">
                {m.value}
                <span className="text-xs md:text-sm text-[#6b7fa8] font-normal ml-1">
                  {m.unit}
                </span>
              </div>
              <div className="text-[10px] md:text-[11px] text-[#6b7fa8] flex items-center gap-2">
                {m.tag && (
                  <span
                    className={`px-1.5 py-0.5 rounded text-[9px] md:text-[10px] font-['Space_Mono'] ${m.tag.bg} ${m.tag.text}`}
                  >
                    {m.tag.label}
                  </span>
                )}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-5 gap-3">
            ...
            <div className="flex gap-4">
              {[
                ...
                <div
                  key={i}
                  className="flex items-center gap-2 text-[10px] md:text-[11px] text-[#6b7fa8] font-['Space_Mono']"
                >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-3 text-center sm:text-left">
          ...
          <div className="font-['Space_Mono'] text-[9px] md:text-[10px] text-[#6b7fa8] flex gap-4">
            <span>
              LAST TICK: <span className="text-amber-400">{lastTick}</span>
            </span>
            <span>
              SAMPLES:{" "}
